# /perf-profile Skill — Design

## Scope

Create the `/perf-profile` Claude Code skill file at `.claude/skills/perf-profile.md`. The skill orchestrates a CPU profiling session using the 7 implemented chrome-function-profiler MCP tools. Generic — works against any app with `performance.mark` pairs, not just the demo app.

## Approach

Strict orchestration: step-by-step guide that tells Claude exactly which MCP tools to call, with exact parameter schemas matching the implemented API. No references to unimplemented features.

## Key Revisions from Plan Spec

The original skill spec (chrome-function-profiler-plan.md lines 1775-1894) had several references to unimplemented features. This design strips those out:

| Plan Spec Reference | Status | Action |
|---------------------|--------|--------|
| `captureNetwork: true` parameter | Not implemented | Removed from all tool calls |
| `analyze_profiles` tool (Step 7 option 4) | Not implemented | Removed entirely |
| `performance.measure` fallback mode | Not implemented | Removed from Step 2 |
| "Network correlation" pattern detection | Not implemented | Removed from Step 6 |
| "First visit vs revisit" / "cumulative degradation" patterns | Not in session summary | Removed from Step 6 |

## Additions to Plan Spec

| Addition | Reason |
|----------|--------|
| Available Tools table | Prevents Claude from hallucinating tool names |
| Full parameter schemas with defaults | Matches actual implemented API exactly |
| Single vs multi-capture result distinction | `profile_scenario` returns top functions; `stop_profiling_session` returns stats/outliers |
| `profile_scenario` blocking behavior note | Implementation polls until capture or timeout — skill should explain this |
...
Single task: create `.claude/skills/perf-profile.md` with the skill content above (the markdown between the triple backticks, not the wrapper doc).
